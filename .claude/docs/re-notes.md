...
| 16 | 0xA817 | level-gate anchor (object 42, `EB 16`, zones 24/49/74/99/124): when the player's x reaches it the end-of-level sequence runs - the framed window (list 0xAAF2: LIVES BONUS lives x 1000 at once, BRAVERY BONUS 10000 ticked up in 80 steps of 125 when the suit is off), for zone 124 the CONGRATULATIONS screen (list 0xAA3F), then the bonus screen at 0xA8DB (list 0xA961: a pointer sweeps rows 13..20 by 0xA960's free-running counter until fire; an odd row pays (row&7) x 1000), an extra life capped at nine, suit off, refills, and the next zone with the entry position from the table at 0xAAE0: 25 (0,0x78), 50 (0,0x20), 75 (0x28,0x80), 100 (0x10,0x70), 0 (0x10,0x70).  The frame tiles live at 0xEAB0 (window) and 0xECE0 (full screen).  Ported in src/gate.mac |
